...
def run():
    ...
    NIFTI_EXT = cf.NIFTI_EXT
    PMAP_EXT = cf.PMAP_EXT
    info("Copying T2* Volumes...")
    t2s_count = 0
    for file in cf.RAW_IMAGES_PATH.glob(f"*{cf.T2S_EXT}"):
        t2s_count += 1
        shutil.copy2(file, cf.T2S_VOLUMES_PATH / file.name)

    info("Copying Ground Truth Labels...")
    gt_lab_count = 0
    for file in cf.RAW_LABELS_PATH.glob(f"*{NIFTI_EXT}"):
        gt_lab_count += 1
    ...
    for file in cf.NNU_INFERENCE_PATH.glob("*"):
        if file.name.endswith(NIFTI_EXT):
            pred_count += 1
            shutil.copy2(file, cf.PRED_PATH / file.name)
        elif file.name.endswith(PMAP_EXT):
            prob_count += 1
